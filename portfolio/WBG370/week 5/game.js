...
$.playground().registerCallback(function(){
	...
	$("#background1").x(($("#background1").x() - closeParallaxSpeed - PLAYGROUND_WIDTH) % (-2 * PLAYGROUND_WIDTH) + PLAYGROUND_WIDTH);
	$(".enemy").each(function(){
		this.enemy.update();
		if(($(this).x() + enemyWidth) < 0){
			...
			if(collided.length > 0){
				...
				if($('#player')[0].player.value >= 100){
					console.log("youWIn");
					document.getElementById("messages").innerHTML = "You Win!";
					$.playground().pauseGame();
				}else if($('#player')[0].player.value <= -100){
...
$(document).keydown(function(e){
  if(e.keyCode === 32){
	  var sound = new Howl({src: ['gunShot.mp3'], volume: 0.1});
	  if($('#player')[0].player.value >= 100 || $('#player')[0].player.value <= -100){
	  sound.pause();
	  }else{sound.play();}
